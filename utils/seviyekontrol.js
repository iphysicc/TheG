...
async function checkLevelUp(message) {
    ...
    if (userXp >= levelUpXp) {
        ...
        const levelUpImage = await generateLevelUpImage(message.author, userLevel, backgroundURL);
...
async function generateLevelUpImage(user, newLevel, backgroundURL = null) {
    ...
    if (backgroundURL) {
        try {
            ...
            background = await Canvas.loadImage(path.join(__dirname, '../assets/8203848.jpg')); 
        }
    } else {
        background = await Canvas.loadImage(path.join(__dirname, '../assets/8203848.jpg')); 
    }
