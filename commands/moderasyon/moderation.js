...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("sebep").setDescription("Atma sebebi.")
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((option) =>
          option.setName("sebep").setDescription("Susturma sebebi.")
        )
    )
    .addSubcommand((subcommand) =>
      subcommand
  ...
  async execute(interaction) {
    ...
    const targetHighestRole = user.roles.highest;
    const targetHighestRoleId = targetHighestRole.id;

    if (targetHighestRoleId > highestRoleId) {
        return interaction.reply({ content: 'Kendinizden yüksek birine işlem uygulayamazsınız!', ephemeral: true });
    }
    ...
    let embed = new EmbedBuilder().setTitle("İşlem Onayı").setColor("#ff0000");

    if (subcommand === "ban") {
      embed.setDescription(
        `${user} kullanıcısını yasaklamak istediğine emin misin?`
      );
    } else if (subcommand === "kick") {
      embed.setDescription(
        `${user} kullanıcısını sunucudan atmak istediğine emin misin?`
      );
    } else if (subcommand === "sustur") {
      embed.setDescription(
        `${user} kullanıcısını susturmak istediğine emin misin?`
      );
    } else if (subcommand === "susturmakaldır") {
      embed.setDescription(
        `${user} kullanıcısının susturmasını kaldırmak istediğine emin misin?`
      );
    } else if (subcommand === "zamanaşımı") {
      const duration = interaction.options.getInteger("süre");
      embed.setDescription(
        `${user} kullanıcısına ${duration} dakika zaman aşımı uygulamak istediğine emin misin?`
      );
    } else if (subcommand === "zamanaşımıkaldır") {
      embed.setDescription(
        `${user} kullanıcısının zaman aşımını kaldırmak istediğine emin misin?`
      );
    }
      ...
      if (i.customId === "confirm") {
        try {
          if (subcommand === "ban") {
            ...
            await i.update({
              content: `${user} başarıyla yasaklandı!`,
              embeds: [],
              components: [],
            });
          } else if (subcommand === "kick") {
            ...
            await i.update({
              content: `${user} başarıyla sunucudan atıldı!`,
              embeds: [],
              components: [],
            });
          } else if (subcommand === "sustur") {
            ...
            await i.update({
              content: `${user} başarıyla susturuldu!`,
              embeds: [],
              components: [],
            });
          } else if (subcommand === "susturmakaldır") {
            ...
            await i.update({
              content: `${user} kullanıcısının susturması kaldırıldı!`,
              embeds: [],
              components: [],
            });
          } else if (subcommand === "zamanaşımı") {
            ...
            await i.update({
              content: `${user} başarıyla ${duration} dakika zaman aşımına uğratıldı!`,
              embeds: [],
              components: [],
            });
          } else if (subcommand === "zamanaşımıkaldır") {
            ...
            await i.update({
              content: `${user} kullanıcısının zaman aşımı kaldırıldı!`,
              embeds: [],
              components: [],
            });
          }
